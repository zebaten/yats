...
void run1 (int jug1, int jug2, int t1, int t2, int g, int *x, int *y, int *jugad);
void run2 (int jug1, int jug2,int jug3, int t1, int t2, int t3, int g);
void run3 (int jug1, int jug2,int jug3,int jug4, int t1, int t2, int t3,int t4, int g);
...
int main (){
    ...
    for (i = 0; i < num; i++){
        do{
            rx = rand () % (tamTabla + 1);
            ry = rand () % (tamTabla + 1);
        }while(tablero[rx][ry]!=0);
        tablero[rx][ry]=valores[i];
    }
    //int numeros = malloc(num * sizeof(int));
    /*for(int i = 0; i < 50; i++){
        if(valores[i] != valores[i-1] && i > 0){
            valores[i] = rand() %max_size +1;
        }else if(i==0){
            valores[i] = rand() %max_size +1;
        }
    }
    ...
    if(jugadores == 2){ 
        if(jug1 > 0 && jug2 >0){ 
            ...
            while(1){
                ...
                printf("coordenada x: %d y:%d jugador:%d",x,y,jugad);
            }
        }
    }
    if(jugadores == 3){ 
        if(jug1 > 0 && jug2 >0 &jug3){ 
            ...
            run2(jug1,jug2,jug3,fd_t1,fd_t2,fd_t3,fd_g);
        }
    }
    if(jugadores == 4){ 
        if(jug1 > 0 && jug2 >0 && jug3 && jug4){ 
            while(1);
        }
        else{
            run3(jug1,jug2,jug3,jug4,fd_t1,fd_t2,fd_t3,fd_t4,fd_g);
        }
    }
    return 0;
}
